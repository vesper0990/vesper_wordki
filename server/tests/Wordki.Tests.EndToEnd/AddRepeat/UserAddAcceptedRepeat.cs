...
{
    [TestFixture]
    public class UserAddCorrectRepeat : TestBase
    {
        public UserAddCorrectRepeat()
        {
        ...
        {
            Request.Content = new StringContent("{\"wordId\":1,\"result\":0}", Encoding.UTF8, "application/json");
        }
        ...
        async Task AndThenRepeatIsAdded()
        {
            using (var dbContext = new EntityFramework(DapperSettings))
            {
                ...
                Assert.AreEqual(0, repeat.Result);
        ...
        async Task AndThenWordIsUpdated()
        {
            using (var dbContext = new EntityFramework(DapperSettings))
            {
                ...
                Assert.AreEqual(Host.TimeProviderMock.Object.GetTime().AddDays(2), word.NextRepeat);
